...
public class JGMR{
    ...
    public static class MyMapper extends Mapper<LongWritable, Text,  Text, Text> {
        ...
        protected void map(LongWritable key, Text value, Context context)
            ...
            context.write(new Text("00"+values[1]), value);
        }
    }
    ...
    public static class MyReducer extends Reducer<Text, Text, Text, Text> {
        ...
        protected void reduce(Text key, Iterable<Text> values,
                              Context context)
                throws IOException, InterruptedException {
            jginfo empJoinDep = new  jginfo();
            Set<String> mark=new HashSet<String>();
            int sum=0;
            for (Text val : values) {
                String[] valuess = val.toString().split("\t");
                if(valuess[3].equals("1000008"))
                {
                    empJoinDep.setXiehui("证券协会");
                }
                else if(valuess[3].equals("1000010"))
                {
                    empJoinDep.setXingye("兴业银行");
                }
...
//        conf.set("yarn.resourcemanager.scheduler.address", "hadoop1:8030");
        ...
        //要处理的数据输入与输出地址
        FileInputFormat.setInputPaths(job, "hdfs://n1:8020/user/database/otc_KHTZZH");
        FileOutputFormat.setOutputPath(job, new Path("hdfs://n1:8020/user/database/otc_KHTZZH/result"));
