    ...
    public static class MyReducer extends Reducer<Text, Text, Text, Text> {
        ...
        protected void reduce(Text key, Iterable<Text> values,
            ...
            for (Text val : values) {
                ...
                /*if(valuess[1].contains("tdx"))
                {
                    mark.add("通达信");
                    context.write(key, new Text("通达信"));
                }*/
               /* if(valuess[1].contains("thinkive"))
                {
                    mark.add("番茄财富");
                    context.write(key, new Text("番茄财富"));
                }*/

                /*if(valuess[1].contains("hxwt"))
                {
                    mark.add("同花顺");
                    context.write(key, new Text("同花顺"));
                }*/
                /*if(valuess[1].contains("sjcg"))
                {
                    mark.add("手机炒股");
                    context.write(key, new Text("手机炒股"));
                }*/

                {
                    mark.add("柜台");
                    context.write(key, new Text("柜台"));
                }
            }
           /* for (String str : mark) {
                result=result+" "+str;
...
//        conf.set("yarn.resourcemanager.scheduler.address", "hadoop1:8030");
        ...
        //要处理的数据输入与输出地址
        FileInputFormat.setInputPaths(job, "hdfs://n1:8020/user/label/aboss/wtgy");
        FileOutputFormat.setOutputPath(job, new Path("hdfs://n1:8020/user/mr/wtgy7"));
